Attribute VB_Name = "Macro_21_AB_import_ventas"


'Attribute VB_Name = "Macro_21_import_Ventas"
Sub Macro_21_AB_import_Ventas()
...
Dim col_ventas_start As Integer
Dim row_ventas_start As Integer
row_ventas_start = 23 ' 'SKU' ROW address
col_ventas_start = 23 ' 'SKU' COLUMN address
...
sh1 = "Ventas STD"
wkbk1 = "02 – VENTAS.xlsm"

' GOTO "VENTAS" WORKBOOK AND SELECT USED RANGE
 Windows(wkbk1).Activate
 Sheets(sh1).Select
 
 'CELL ADDRESS WHERE THE RANGE TO IMPORT START
 Cells(row_ventas_start, col_ventas_start).Select 'Range("T23").Select 'familas
 ...
 Range(Cells(row_ventas_start - 2, col_ventas_start - 1), Cells(rw1 - 1, col_ventas_start + 1)).Select
 Selection.Copy
